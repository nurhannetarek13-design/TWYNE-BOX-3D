...
await import('./gift-set-v36.js?build=49');THREE.Group.prototype.add=A;
...
function tex(t,fs,tr){const c=document.createElement('canvas');c.width=1800;c.height=360;const x=c.getContext('2d');x.clearRect(0,0,1800,360);x.fillStyle='#171716';x.textBaseline='middle';x.font=`600 ${fs}px Manrope,Arial,sans-serif`;const a=[...t],w=a.map(q=>x.measureText(q).width),T=w.reduce((p,q)=>p+q,0)+tr*(a.length-1);let p=(1800-T)/2;a.forEach((q,i)=>{x.fillText(q,p,180);p+=w[i]+tr});const m=new THREE.CanvasTexture(c);m.colorSpace=THREE.SRGBColorSpace;m.minFilter=THREE.LinearFilter;m.magFilter=THREE.LinearFilter;m.generateMipmaps=false;m.needsUpdate=true;return m;}
function add(t,w,h,z,fs,tr,ord){const m=new THREE.Mesh(new THREE.PlaneGeometry(w,h),new THREE.MeshBasicMaterial({map:tex(t,fs,tr),transparent:true,toneMapped:false,depthTest:false,depthWrite:false,side:THREE.DoubleSide}));m.rotation.x=-Math.PI/2;m.position.set(0,20.99,z);m.renderOrder=52000+ord;R.sg?.add(m);}
add('VOLUME I — KENOPSIA',82,6.0,30.8,86,10,1);
add('EAU DE PARFUM',58,5.0,38.5,82,8,2);

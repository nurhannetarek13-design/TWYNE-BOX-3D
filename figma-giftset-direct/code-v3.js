...
function addInside(parent) {
  ...
  PIECES.forEach(function (p, i) {
    ...
    addText(parent, p.notes, mm(leftMM - 1), mm(170), mm(groupWidthMM + 2), mm(10), {
      size: 6.1,
      align: 'CENTER',
      color: INK,
      tracking: 0.62,
...
function makeGuide() {
  ...
  PIECES.forEach(function (p, i) {
    ...
    addText(sheet, p.notes, mm(centerMM - 21), mm(168), mm(42), mm(10), {
      size: 4.6,
      align: 'CENTER',
      color: INK,
      tracking: 0.28,
...
Promise.all([
  ...
  spec.characters = 'OPEN 186 × 208 MM  /  FOLDED 186 × 104 MM  /  SCORE AT 104 MM  /  CREAMY #ECE8DF  /  BOTTLE CENTERS 23 / 70 / 116 / 163 MM  /  NOTES VISIBLE UNDER EVERY BOTTLE';
  spec.fontSize = 7;
  ...
  figma.closePlugin('TWYNE Gift Set updated — visible notes + Aoyama-style typography');
}).catch(function (err) {
